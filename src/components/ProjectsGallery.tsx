...
import { X, ExternalLink, Github, Filter, Globe, Smartphone, Brain, Database, Code, Star, Calendar } from 'lucide-react'
import type { Project } from '../lib/supabase'
import { fetchProjects } from '../lib/supabase'

interface ProjectsGalleryProps {
  limit?: number // Nombre de projets à afficher (undefined = tous)
  showFilters?: boolean // Afficher les filtres de catégories
  showHeader?: boolean // Afficher le header avec titre
}
...
const ProjectsGallery: React.FC<ProjectsGalleryProps> = ({ 
  ...
  useEffect(() => {
    filterProjects()
  }, [selectedCategory, projects, limit])
  ...
  return (
    <section className="py-12 sm:py-16 md:py-20 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="container mx-auto relative z-10 max-w-7xl">
        {/* Header - Responsive - Conditionnel */}
        {showHeader && (
          <motion.div
        ...
        {/* Filtres - Responsive avec scroll horizontal sur mobile - Conditionnel */}
        {showFilters && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
                ...
                {categories.map((category) => {
                  ...
                  return (
                    <motion.button
                      key={category.id}
                      onClick={() => setSelectedCategory(category.id)}
                      className={`flex-shrink-0 px-4 py-2 rounded-full font-medium transition-all flex items-center gap-2 text-sm whitespace-nowrap ${
                        selectedCategory === category.id
          ...
          >
            <AnimatePresence mode="popLayout">
              {filteredProjects.map((project, index) => (
                ...
                >
                  <div className="glass-effect rounded-xl sm:rounded-2xl overflow-hidden hover-lift cursor-pointer">
                    ...
                    <div className="relative h-48 sm:h-56 md:h-64 overflow-hidden">
                      ...
                      {project.featured && (
                        <div className="absolute top-3 sm:top-4 left-3 sm:left-4">
                          <span className="px-2 sm:px-3 py-1 bg-gradient-to-r from-yellow-500 to-orange-500 text-white text-xs font-bold rounded-full flex items-center gap-1">
                      ...
                      {/* Actions rapides - Visible sur hover desktop, toujours visible mobile */}
                      <div className="absolute top-3 sm:top-4 right-3 sm:right-4 flex gap-2 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                        {project.githuburl && (
                          <motion.a
                            href={project.githuburl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="w-8 h-8 sm:w-10 sm:h-10 glass-effect rounded-full flex items-center justify-center hover:scale-110 transition-transform"
                    ...
                    <div className="p-4 sm:p-5 md:p-6">
                      <h3 className="text-lg sm:text-xl font-bold mb-2 gradient-text line-clamp-1">
                        {project.title}
                      </h3>
                      ...
                      <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-3 sm:mb-4">
                        {project.technologies.slice(0, 3).map((tech) => (
                          <span
                            key={tech}
                            className="px-2 sm:px-3 py-0.5 sm:py-1 glass-effect text-xs font-medium rounded-full"
                          >
                            {tech}
                          </span>
                        ))}
                        {project.technologies.length > 3 && (
                          <span className="px-2 sm:px-3 py-0.5 sm:py-1 text-xs text-gray-500">
                            +{project.technologies.length - 3}
                          </span>
                        )}
                      </div>
        ...
        {selectedProject && (
          ...
          >
            ...
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="relative max-w-4xl w-full max-h-[90vh] glass-effect-dark rounded-3xl overflow-hidden flex flex-col"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Header avec image */}
              <div className="relative h-80 flex-shrink-0">
                ...
                <button
                  onClick={() => setSelectedProject(null)}
                  className="absolute top-4 right-4 w-10 h-10 glass-effect rounded-full flex items-center justify-center hover:scale-110 transition-transform z-10"
                >
                  <X size={20} />
                </button>
              </div>
...
//       {/* Modal détail projet - Largeur fixe avec scroll interne */}
//       <AnimatePresence>
//         {selectedProject && (
//           <motion.div
//             initial={{ opacity: 0 }}
